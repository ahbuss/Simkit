...
/**
 * Combination factory object for {@code EventListImpl} as well as a static
 * interface to a singleton default instance of {@code EventListImpl}.
 * <p>
 * Schedule maintains the various <CODE>EventListImpl</CODE> instances and
 * provides a universally available way to interact with a single default
 * instance of an <CODE>EventList</CODE>.
 *
 * @author K. A. Stork
 * @author Arnold Buss
 * @version $Id$
...
public class Schedule {
    ...
    private static final Map<Integer, BasicEventList> allEventLists;
    ...
    public static boolean isVerbose() {
        return defaultEventList.isVerbose();
    }

    /**
     * Sets single step mode. After each event is executed, the user must enter
     * something at the console.<BR>
    ...
    public static boolean isSingleStep() {
        return defaultEventList.isSingleStep();
    }

    /**
     * @return True when a simulation is currently running.
     */
    public static boolean isRunning() {
        return defaultEventList.isRunning();
    }
    ...
    /**
     * Starts or resumes simulation. This method implements the fundamental
     * discrete event simulation algorithm. While the Event List is not empty,
     * simTime is advanced to the time of the soonest event, which is then
     * processed. If the Event List becomes empty, then the simulation stops and
     * the method returns.
     *
     */
     ...
     * time. Any previously set stop time or stop on event is cleared.
     *
     ...
     * event list is printed. Used by dump() and getEventListAsString()
     *
    ...
    /**
     * Adds the entity to the list of entities with "Run" events. Note:
     * addRerun() no longer schedules the Run event - that is supposed to happen
     * when Schedule.reset() is invoked, as it <i>should</i>
     * be, before each simulation run. schedules the Run event at the current
     * time. (For default event list)
     *
    ...
    /**
     * Removes the SimEntity from the list of entities with Run events. Note it
     * does not interrupt its Run event.
     *
     * @param se SimEntity to be removed
     */
    public static void removeRerun(ReRunnable se) {
        defaultEventList.removeRerun(se);
    }

    /**
     * Clears the list of SimEntities with Run events.
     */
    public static void clearRerun() {
        defaultEventList.clearRerun();
    }
     ...
     *
     * @param ignoredEventName The event that was previously ignored but now is
     * not.
     *
     */
     ...
     */
    public static boolean isReallyVerbose() {
        return defaultEventList.isReallyVerbose();
    }
...
//        return key.intValue();
    ...
    /**
     * Instantiates a new EventList subclass. Assumes that the constructor has
     * signature <CODE>(int)</CODE>
     *
    ...
    /**
     * Will return an existing EventList, if one with that id exists. Otherwise,
     * will create a new one and return a reference to it.
     *
    ...
    public static BasicEventList getEventList(int index) {
        ...
        if (eventList == null) {
            ...
            allEventLists.put(index, eventList);
        }
        return eventList;
    }

    /**
     * @return Reference to current default event list
     */
    ...
    /**
     * If true, then only one event will be processed. A call to
     * <CODE>startSimulation()</CODE> is required to get going again.
     *
     ...
     *
     * @param printStream the PrintStream instance for the default EventList to
     * dump to.
     */
