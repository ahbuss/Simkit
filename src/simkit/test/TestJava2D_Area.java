 ...
 *
 * @author  Arnold Buss
 * @version $Id: TestJava2D.java 1083 2008-06-11 20:41:21Z kastork $
 */
public class TestJava2D_Area extends JPanel{
    ...
    public TestJava2D_Area() {
        ...
        Shape myElipse = new Ellipse2D.Double(100.0, 100.0, 300.0, 300.0);

        shape[0] = new Area(myElipse);
        
        Shape[] seg = Math2D.getSegments(shape[0], null);
...
//        Ellipse2D ellipse = (Ellipse2D) shape[0];
        ...
        CubicCurve2D segment  = (CubicCurve2D) Math2D.getSegments(shape[0], null)[0];
        ...
        for (Shape s : intersections){
            Point2D iPoint = s.getBounds().getLocation();
            System.out.println(iPoint);
            System.out.println(Math.toDegrees(pointToBearing(iPoint)));
        }
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < point.length; i++) {
            ...
            Point2D[] intersect = Math2D.findIntersection(
                    point[i],
                    Math2D.subtract(point[(i + 1) % point.length], point[i]),
                    shape[0]);
    ...
    public static void main(String args[]) {
        ...
        JPanel panel = new TestJava2D_Area();
    ...
    public static final double PiOver2 = StrictMath.PI / 2.0;

    public static double pointToBearing(Point2D location) {
        double ret = StrictMath.atan2(location.getX(), location.getY());
        if (ret < 0.0) {
            ret += TWO_PI;
        }
        return ret;
